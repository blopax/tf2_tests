...
"""
fit can be used if dataset not too large --> otherwise use generator
...
ds_t = tf.data.Dataset.from_tensor_slices(x)
ds_x = tf.data.Dataset.from_tensors(x)
ds_y = tf.data.Dataset.from_tensors(y)
